...
//
// onAnimChange (optional) lets /flash mirror the current short into the URL. It
// fires from an effect rather than from next(), because the timer advances the
// channel too — an effect covers the first pick, the button, and the timer.
const ADVANCE_MS = 90000; // ~90s per short

export default function FlashChannel({ onAnimChange }) {
  ...
  // Report every change to the owner (the URL mirror), including the first pick.
  useEffect(() => {
    if (anim) onAnimChange?.(anim);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [anim]);

  if (!anim) return <p className="arcade-channel-loading">tuning in…</p>;

  // Share the short on screen — see the same note in FlashTheater.
