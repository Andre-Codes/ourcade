...
/* PhonePanel — the Nopia itself: the emulator iframe (public/games/snake.html
   with ?personal=1) plus the THIN ADAPTER that bridges the always-on
   PhoneProvider context ↔ that sandboxed iframe. The iframe can't import
   Firebase, so PhoneProvider holds the live listeners and this panel just relays
   context → iframe (identity + snapshots + ring signals) and iframe → context
   (send/addcontact/ping/read/clearmessages).
   ...
   provider; rendering this panel just shows the live state.

   Two consumers share it: the full-page /phone route (PhonePage) and the
   site-wide pop-up overlay (PhoneOverlay). */
...
export default function PhonePanel() {
  ...
  // don't want a replay ring just for showing this panel).
